...
setup:
	virtualenv -p `which python3.5` $(WORKON_HOME)/teqtest
	$(WORKON_HOME)/teqtest/bin/pip install -U pip wheel
	$(WORKON_HOME)/teqtest/bin/pip install -Ur requirements/dev.txt
	$(WORKON_HOME)/teqtest/bin/pip freeze
	npm install
	npm update
	cp teqtest/settings/local.example.py teqtest/settings/local.py
	echo "DJANGO_SETTINGS_MODULE=teqtest.settings.local" > .env
	createdb -E UTF-8 teqtest
	$(WORKON_HOME)/teqtest/bin/python manage.py migrate
	if [ -e project.travis.yml ] ; then mv project.travis.yml .travis.yml; fi
	@echo
	@echo "The teqtest project is now setup on your machine."
...
update:
	$(WORKON_HOME)/teqtest/bin/pip install -U -r requirements/dev.txt
	npm install
	npm update
