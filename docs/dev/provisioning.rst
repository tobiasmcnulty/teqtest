...
Teqtest is deployed on the following stack.
...
documented on that page, you need to create a new file in ``teqtest/celery.py`` and then
modify ``teqtest/__init__.py`` to import that file. You'll also need to customize ``{{
...
``requirements/base.txt`` and installed it)::
    ...
    celery -A teqtest worker

Additionally you will need to uncomment the ``BROKER_URL`` setting in the project settings::

    # teqtest/settings/deploy.py
    from .base import *

    # ...
    BROKER_URL = 'amqp://teqtest_%(ENVIRONMENT)s:%(BROKER_PASSWORD)s@%(BROKER_HOST)s/teqtest_%(ENVIRONMENT)s' % os.environ
...
the broker by default. The RabbitMQ user will be named ``teqtest_<environment>`` and the
vhost will be named ``teqtest_<environment>`` for each environment. It requires that you
...
You can use the below OpenSSL commands to generate the key and signing request::
  ...
  openssl genrsa -out teqtest.key 2048
  # Make copy of the key with the passphrase
  cp teqtest.key teqtest.key.secure
  # Remove any passphrase
  openssl rsa -in teqtest.secure -out teqtest.key
  # Generate signing request
  openssl req -nodes -sha256 -new -key teqtest.key -out teqtest.csr
